...
| D7a | **Deploy found it** — no loading state anywhere, so a click showed 2s of nothing on a money screen | **new:** `src/components/payment-summary-card.tsx`, `ops/payments/loading.tsx`, `ops/payments/[paymentId]/loading.tsx` · **modified:** `src/app/ops/page.tsx` (9) | ✅ 2026-09-24 |
| D2 | deploy blocker cleared — a rail timeout, so a SLOW rail cannot hang the ops deal book | **modified:** `src/lib/reconciliation/queue.ts` (mine) | ✅ 2026-09-24 · proved against a black-holed host: 4011 ms → `unreachable` |
